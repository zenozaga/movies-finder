...
var cast_1 = __importStar(require("../models/cast"));
...
var TMDBAPI = /** @class */ (function (_super) {
    ...
    TMDBAPI.prototype.parseSource = function (html) {
        ...
        if (providerImage.length) {
            ...
            if (image)
                image = "".concat(this.site).concat(image);
            var name = providerImage.attr("alt");
            var networkLink = doc("[haref*=network/]").attr("href");
            if (networkLink)
                networkLink = "".concat(this.site).concat(networkLink);
            var networkImage = doc("[haref*=network/] img").attr("src");
            if (networkImage)
                networkImage = "".concat(this.site).concat(networkImage);
        ...
        if (socialLinks.length) {
            socialLinks.each(function (index, ele) {
                var a = doc(ele);
